...
extern "C" {
  ...
  LLVMBool LLVMRunOurModulePasses(LLVMModuleRef M) {
    ...
    MPM->add(createCFGSimplificationPass());
    MPM->add(createSimpleLoopUnrollPass(4));    // Unroll small loops
    ...
    MPM->add(createLICMPass());
    MPM->add(createLoopUnswitchPass(1, 0));//tune this
    MPM->add(createCFGSimplificationPass());
    MPM->add(createInstructionCombiningPass(1));
    ...
    MPM->add(createLoopUnrollPass(4, 400, 0, 1, 1, 16));    // Unroll small loops
