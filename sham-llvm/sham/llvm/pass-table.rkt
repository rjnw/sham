...
(define (cleanup s)
  (cond [(symbol? s) (cleanup (symbol->string s))]
        [(string? s) (string-downcase
                      (string-replace (string-replace s "_" "")
                                      "-" ""))]
        [else (error 'sham:pass/attribute "unknown pass/attribute name, requires string/symbol. ~a" s)]))

(define (lookup-pass p)
  (hash-ref pass-table (cleanup p)))

(define (lookup-attribute a context)
  (LLVMCreateEnumAttribute
   context
   (LLVMGetEnumAttributeKindForName (cleanup a))
   0))
