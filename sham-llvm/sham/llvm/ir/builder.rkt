...
(define (build-llvm-module module-ast
    ...
    (match def
      [(llvm:def:type info name t)
       (if (llvm:ast:type:struct? t)
           (assoc-env-extend decl-env (to-string name) (LLVMStructCreateNamed (to-string name)))
           decl-env)]
      [(llvm:def:function info name args arg-types ret-type body)
       ...
       (assoc-env-extend decl-env (to-string name) (llvm-function value type))]
      [(llvm:def:global info name t v)
       ...
       (assoc-env-extend decl-env (to-string name) (llvm-value value type))]))
  ...
  ;; llvm:ast:expr:constant env -> LLVMValueRef
  (define (compile-constant v internal-env decl-env)
    (match v
      ...
      [(llvm:ast:constant:struct md vals t)
       (LLVMConstStruct (map (curryr compile-constant internal-env decl-env) vals))]
      [(llvm:ast:constant:array md vals t)
       (LLVMConstArray (compile-type t internal-env decl-env)
                       (map (curryr compile-constant internal-env decl-env) vals))]
      ...
      [(llvm:ast:constant:vector md vals)
       (LLVMConstVector (map (curryr compile-constant internal-env decl-env) vals))]))
  ...
  (define (compile-define def internal-env decl-env)
    (define (compile-function-definition! def name args types ret-type blocks)
      ...
      (define (local-var-ref sym) (hash-ref local-var-map sym #f))
      ...
      (define (build-block! b)
        ...
        (build-initial-instruction! (car instructions))
        (map build-instruction! (cdr instructions))
