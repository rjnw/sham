  ...
  (define (compile-module-statement stmt env module-env)
    (define (compile-function-definition name args types ret-type body)
      ...
      (define (build-expression e env)
        ...
        (match e
          [(sham:exp:let md ids types vals st ex)
           (define new-env
             (for/fold ([env env])
                       ([id-type types]
                        [id ids]
          ...
          [(sham:exp:var md sym)
           (LLVMBuildLoad jit-builder
                          ...
                          (symbol->string sym))]
          [(sham:exp:external md lib-id sym t)
           (define value
