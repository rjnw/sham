...
(build-recursion
 ...
 (cexpr (stx -> stx)
        ...
        [('cond [chk:expr thn:expr] ... ['else els:expr]) (expr:ifcond (chk thn) ... els)]
        [(rator:expr rands:expr ...) (expr:app rator rands ...)])

 (type (stx -> stx)
       [(~seq (~optional {pvars:id ...})
              (~optional (~seq cs:expr ... '=>))
              ts:type)
        (type:poly (pvars ...) (cs ...) ts)]
       [(~seq ffrom:type (~seq '-> tto:type) ...) (type:func ffrom (list tto ...))]
       [#(t:type ...) (type:tuple t ...)]
